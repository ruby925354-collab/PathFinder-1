...
const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  const [knowledgeScore, setKnowledgeScore] = useState<{ subject: string; score: number } | string | null>(null);
  ...
  // 🟤 Fetch Profile, Result, and Feedback
    ...
    const fetchProfileData = async () => {
      ...
      try {
        ...
        // ✅ Safely parse strong knowledge area
        const strongArea = data.strong_knowledge_area;
        if (strongArea && typeof strongArea === 'object' && 'subject' in strongArea) {
          setKnowledgeScore(strongArea);
        } else if (typeof strongArea === 'string') {
          const match = strongArea.match(/^([A-Za-z_ ]+)\s*(\d+)?/);
          if (match) {
            const subject = match[1].trim().replace(/_/g, ' ');
            const score = match[2] ? parseInt(match[2], 10) : 0;
            setKnowledgeScore({ subject, score });
          } else {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 backdrop-blur-sm">
      <div className="bg-gradient-to-b from-brown-1 to-gray-200 rounded-3xl shadow-2xl w-full h-full md:w-[75vw] md:h-[90vh] relative overflow-hidden flex flex-col md:flex-row border border-brown-3">
        {/* Sidebar */}
        <div className="bg-brown-6 text-white flex flex-col md:w-1/3 w-full rounded-t-3xl md:rounded-l-3xl shadow-2xl overflow-hidden">
        ...
        {/* Main Content */}
        <div className="flex-1 relative overflow-y-auto bg-white p-4 md:p-8 rounded-b-3xl md:rounded-r-3xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-x-10 md:gap-y-8">
                    ...
                    <div className="flex flex-col col-span-2">
                      ...
                      <input
                        value={user?.email || ''}
                        disabled
                        className="p-3 md:p-4 rounded-xl border border-[#CBB197] bg-white text-gray-800 font-medium shadow-sm focus:ring-2 focus:ring-[#B08968] transition-all"
                      />
                    </div>
              ...
              {!selectedProgram ? (
                <>
                  ...
                  <div className="flex flex-col md:flex-row justify-center items-stretch gap-10 w-full max-w-4xl">
                    {/* Personality Card */}
                    <div className="flex flex-col flex-1 items-center bg-white/70 backdrop-blur-md p-8 rounded-3xl border border-[#E1C9A7] shadow-lg hover:shadow-xl transition-all duration-300">
                      <p className="text-lg font-medium text-[#6B4226] mb-2 uppercase tracking-wide">
                        Personality Type
                      </p>
                      <div className="bg-[#E7D4C0] px-10 py-4 rounded-full text-[#5C3D2E] text-2xl font-bold shadow-inner border border-[#CBB197]">
                        {personalityType || 'N/A'}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col flex-1 items-center bg-white/70 backdrop-blur-md p-8 rounded-3xl border border-[#E1C9A7] shadow-lg hover:shadow-xl transition-all duration-300">
                      ...
                      <div className="bg-[#E7D4C0] px-10 py-4 rounded-full text-[#5C3D2E] text-2xl font-bold shadow-inner border border-[#CBB197]">
                        {typeof knowledgeScore === 'object' && knowledgeScore !== null
                          ? `${knowledgeScore.subject} (${knowledgeScore.score}%)`
                          : typeof knowledgeScore === 'string'
                          ? knowledgeScore
                          : 'N/A'}
                      </div>
                    </div>
                ...
                {submittedFeedback ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-brown-6 text-white rounded-xl hover:bg-brown-7 transition-all"
                    >
                      Submit Feedback
                    </button>
                  </>
